...
class FlutterAdPreloader {
  ...
  void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result) {
    switch (call.method) {
      case "MobileAds#startPreloading": {
        ...
        if (request != null) {
          AdRequest adRequest;
          if (request instanceof FlutterAdManagerAdRequest) {
            adRequest = ((FlutterAdManagerAdRequest) request).asAdManagerAdRequest(adUnitId);
          } else {
            adRequest = request.asAdRequest(adUnitId);
          }
          builder.setAdRequest(adRequest);
        }
      ...
      case "MobileAds#getPreloadConfigurations": {
        ...
        if (configs != null) {
          for (Map.Entry<String, PreloadConfiguration> entry : configs.entrySet()) {
            response.put(entry.getKey(), serializeConfig(entry.getValue()));
          }
        }
      ...
      case "MobileAds#pollAd": {
        ...
        if (className.equals("InterstitialAd")) {
          ...
          if (preloadedAd != null) {
            String adUnitId = preloadedAd.getAdUnitId();
            FlutterInterstitialAd adWrapper =
          ...
          AdManagerInterstitialAd preloadedAd = (AdManagerInterstitialAd)
                  InterstitialAdPreloader.pollAd(preloadId);
          if (preloadedAd != null) {
            String adUnitId = preloadedAd.getAdUnitId();
            FlutterAdManagerInterstitialAd adWrapper =
  ...
  private Map<String, Object> serializeConfig(PreloadConfiguration config) {
    if (config == null) {
      return null;
    }
    Map<String, Object> map = new HashMap<>();
    map.put("adUnitId", config.getAdUnitId());
