...
class GmaMediationChartboostPluginTests: XCTestCase {
  ...
  func testClearDataUseConsent() {
    let chartboostApiFake = ChartboostApiFake()

    GmaMediationChartboostPlugin(chartboostApi: chartboostApiFake)
      .clearDataUseConsent(privacyStandard: .gdpr)

    XCTAssertEqual(chartboostApiFake.clearDataUse, 1)
  }
}
